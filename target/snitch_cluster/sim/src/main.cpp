...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (a == "--params" && i + 1 < argc) i++;          // label only, ignored
        else if (a == "--verify") { /* always on; accepted for back-compat */ }
        else if (a == "--timing-only") verify = false;          // escape hatch: skip the check
        else if (a == "--timeline" && i + 1 < argc) timeline_path = argv[++i];
        else if (a[0] != '-') elf_path = argv[i];
    }
    if (!elf_path) {
    ...
    if (!m.tohost)
        std::fprintf(stderr, "memsim: warning: no 'tohost' symbol; exit undetectable\n");
    ...
    world.set_goldens(img.sym("M2_oscore_expected"), img.sym("M2_suc_expected"),
                      img.sym("M2_iscore_expected"));
    // SSM shape params live as uint32 globals in data.h; read their VALUES from .data.
    auto symval = [&](const char* n) { uint32_t a = img.sym(n); return a ? m.mem.ld32(a) : 0u; };
    world.set_ssm(symval("dState"), symval("xProjDim"), symval("bc_pad_banks"),
                  symval("dtRankUnroll"));
        ...
        if (!f) {
            ...
            for (const auto& s : world.trace())
                std::fprintf(f, "%s,%llu,%llu,%llu\n", NAMES[s.engine],
                             (unsigned long long)s.start, (unsigned long long)s.end,
                             (unsigned long long)s.ideal);
            std::fclose(f);
            std::fprintf(stderr, "memsim: wrote timeline (%zu segments) to %s\n",
                         world.trace().size(), timeline_path.c_str());
        }
    }
    ...
    if (std::getenv("MEMSIM_DEBUG")) {
        std::fprintf(stderr,
            ...
            img.entry, m.tohost, m.exited, m.exit_code,
            m.harts[0].pc, (unsigned long long)m.harts[0].cycle,
    ...
    if (verify)
        std::fprintf(stderr, "AGU layout audit: %u located error(s) over %d invocation(s) "
                     "(bounds + producer->consumer + writer no-alias)\n",
                     world.n_layout_errs(), world.layout_invocations());
    if (!verify)
        std::printf("Note: LAYOUT/BIST check skipped (--timing-only); the app's FP "
