...
uint32_t SimWorld::dma_submit(const DmaDesc& d, uint64_t at) {
    ...
    if (std::getenv("MEMSIM_DMA") && dma_txid_ <= 8) {
        std::fprintf(stderr, "  dma#%u src=0x%08x dst=0x%08x size=%u 2d=%d rep=%u beats=%llu busy_until=%llu\n",
                     dma_txid_, src, dst, d.size, d.is_2d, d.repeat,
                     (unsigned long long)beats, (unsigned long long)dma_busy_until_);
    }
    return dma_txid_;
}
...
void SimWorld::snax_write(uint32_t csr, uint32_t val, uint64_t at) {
    ...
    switch (csr) {
        case SNAX_STREAMER_START:      break;   // functional: run happens at simbacore start
        case SNAX_DELAYED_START_R10:   break;   // functional no-op (pacing only)
        case SNAX_DELAYED_START_R11:   break;
        case SNAX_MODE:    cfg_.mode = val; break;
        case SNAX_SEQ_LEN: cfg_.seqLen = val; break;
        case SNAX_D_MODEL: cfg_.dModel = val; break;
        case SNAX_DT_RANK: cfg_.dtRank = val; break;
        case SNAX_D_INNER: cfg_.dInner = val; break;
        case SNAX_D_FINAL: cfg_.dFinal = val; break;
        case SNAX_SIMBACORE_START: if (val) run_invocation(at); break;
        default: break;
    }
}
...
uint32_t SimWorld::snax_read(uint32_t csr, uint64_t at, bool& is_poll) {
    is_poll = false;
    switch (csr) {
        case SNAX_STREAMER_BUSY:    is_poll = true; return at < accel_end_ ? 1 : 0;
        case SNAX_SIMBACORE_BUSY:   is_poll = true; return at < accel_end_ ? 1 : 0;
        case SNAX_STREAMER_PERF:    return 0;
        case SNAX_SIMBACORE_PERF:   return perf_;
        case SNAX_R10_GAUGE:        is_poll = true;
            return gauge_at(at, accel_start_, t_oscore_done_, g_r10_total_);
        case SNAX_R11_GAUGE:        is_poll = true;
            return gauge_at(at, t_oscore_done_, t_suc_done_, g_r11_total_);
        case SNAX_ISCORE_TILE_CNT:  is_poll = true;
            // P2: isCore runs last (after SUC); P1: overlaps osCore from start.
            return gauge_at(at, phase2_ ? t_suc_done_ : accel_start_, accel_end_,
                            g_iscore_total_);
        default:
            if (csr >= SNAX_STREAMER_CFG_LO && csr <= SNAX_STREAMER_CFG_HI)
                return raw_[csr - SNAX_STREAMER_CFG_LO];
            return 0;
    }
}
...
void SimWorld::run_invocation(uint64_t at) {
    ...
    if (en_suCore) {
        ...
        if (std::getenv("MEMSIM_ACC"))
            std::fprintf(stderr, "  SUC per-cycle: suc=%llu "
    ...
    if (is_simd) {
        ...
        GemmResult g = cyc_gemm(rd, nch, nsp, 2, ports_[14], 1, 1,
                                (long)M_i * osN, (long)cfg_.dModel, 0);
        t_oscore_done_ = t_suc_done_ = at;
        accel_end_ = at + g.end;
        ...
        GemmResult g = cyc_gemm(nullptr, nullptr, nullptr, 0, ports_[17], 4, 1,
                                (long)M_i * cfg_.dFinal, (long)K_i, 0);
        t_oscore_done_ = t_suc_done_ = at;
        accel_end_ = at + g.end;
        ...
        uint64_t fill = (sw_mode == 1) ? ((uint64_t)M_i * fill_p1_per_mtile_ + fill_p1_base_)
                                       : fill_is_osgemm_;
    ...
    if (trace_on_) {
        ...
        for (int p = 0; p < N_PORTS; p++) {
            ...
            uint64_t steps = 1;
            for (int d = 0; d < 4; d++)
                steps *= ports_[p].t_bound[d] ? (uint64_t)ports_[p].t_bound[d] : 1;
    ...
    if (verify_ && phase2_ && !verify_ran_) { verify_ran_ = true; verify_datapath(); }

    if (std::getenv("MEMSIM_ACC"))
        std::fprintf(stderr, "  accel mode=%u P%d seqLen=%u dModel=%u dInner=%u dFinal=%u "
                     "osc=%llu suc=%llu isc=%llu dur=%u busy=%llu\n",
                     cfg_.mode, phase2_ ? 2 : 1, cfg_.seqLen, cfg_.dModel, cfg_.dInner,
                     cfg_.dFinal, (unsigned long long)osc_dur_, (unsigned long long)suc_dur_,
                     (unsigned long long)isc_dur_, perf_,
...
static bool cmp_fp32_golden(const std::vector<double>& model, const std::vector<double>& gold,
                            const char* label, int cols, double TOL, double ABS, double max_bad_frac) {
    std::vector<double> ratios;
    for (size_t i = 0; i < gold.size(); i++)
        if (std::fabs(gold[i]) > 1e-3) ratios.push_back(model[i] / gold[i]);
    double scale = 1.0;
    if (!ratios.empty()) { std::sort(ratios.begin(), ratios.end()); scale = ratios[ratios.size() / 2]; }
    int bad = 0, nfm = 0, fm[8];
    for (size_t i = 0; i < gold.size(); i++) {
        double ref = scale * gold[i], d = std::fabs(model[i] - ref);
        if (d > ABS && d / (std::fabs(ref) + 1e-3) > TOL) { if (nfm < 8) fm[nfm++] = (int)i; bad++; }
    }
    ...
    std::fprintf(stderr, "FP32 %s vs golden: requant scale=%.4g, %d/%zu within %.0f%% -> %s%s\n",
                 label, scale, (int)gold.size() - bad, gold.size(), TOL * 100,
                 ok ? "MATCH" : "MISMATCH", collapsed ? " (scale collapse!)" : "");
    for (int i = 0; i < nfm; i++)
        std::fprintf(stderr, "    %s @ idx %d (r=%d,c=%d): model=%.3f scale*golden=%.3f\n",
                     label, fm[i], fm[i] / cols, fm[i] % cols, model[fm[i]], scale * gold[fm[i]]);
    return ok;
}
...
void SimWorld::verify_layout() {
    ...
    auto extent = [&](int p, uint32_t& lo, uint32_t& hi) {
        ...
        if (ns == 2) { for (int s = 0; s < 2; s++) { int64_t v = a.s_stride[s]; (v >= 0 ? mx : mn) += v; } }
        else { int64_t v = (int64_t)(NCH[p] - 1) * a.s_stride[0]; (v >= 0 ? mx : mn) += v; }
        lo = (uint32_t)((int64_t)a.base + mn);
        hi = (uint32_t)((int64_t)a.base + mx);
    };
    uint32_t rlo[N_PORTS] = {0}, rhi[N_PORTS] = {0};
    for (int p = 0; p < N_PORTS; p++) if (ports_[p].enabled) extent(p, rlo[p], rhi[p]);
    layout_invs_++;
    auto report = [&](int p, const char* kind, uint32_t lo, uint32_t hi) {
        ...
        if (layout_print_budget_-- > 0)
            std::fprintf(stderr, "  LAYOUT ERROR (inv %d): %s%d %s addr [%08x,%08x]\n",
                         layout_invs_, p < N_READERS ? "R" : "W",
                         p < N_READERS ? p : p - N_READERS, kind, lo, hi);
    };
                ...
                for (long i2 = 0; i2 < b[2]; i2++)
                    for (long i3 = 0; i3 < b[3]; i3++)
                        for (long s = 0; s < ns; s++, iter++) {
                            uint32_t addr = (uint32_t)((int64_t)a.base + i0 * a.t_stride[0] +
                                i1 * a.t_stride[1] + i2 * a.t_stride[2] + i3 * a.t_stride[3] +
                                s * a.s_stride[0]);
                            if (!seen.insert(addr).second) {  // already written -> alias
                                if (do_report) {
                                    ...
                                    layout_pass_ = false;
                                    if (layout_print_budget_-- > 0)
                                        std::fprintf(stderr, "  LAYOUT ERROR (inv %d): W%d ALIASES "
    ...
    if (std::getenv("MEMSIM_LAYOUT_FAULT") && layout_invs_ == 1) {
        int r = -1;
        for (int p = 0; p < N_READERS; p++) if (ports_[p].enabled) { r = p; break; }
        if (r >= 0) {
            ...
            std::fprintf(stderr, "  AGU audit self-test: R%d outer-stride+0x100000 -> hi=%08lx -> %s\n",
                         r, (unsigned long)(uint32_t)hi, caught ? "LOCATED (audit is live)" : "MISSED (audit DEAD)");
        }
        ...
        Agu fake;
        fake.t_stride[0] = 8; fake.t_bound[0] = 4;     // both non-zero, equal -> aliasing
        fake.t_stride[1] = 8; fake.t_bound[1] = 4;
...
void SimWorld::verify_datapath() {
    ...
    if (M % Mu || N % Nu) { return; }                       // not an osCore GEMM shape
    ...
    bool r0_ok = r0.t_stride[0] == Mu && r0.t_stride[1] == K * Mu &&
                 (r0.s_stride[0] == 0 || r0.s_stride[0] * 2 == Mu);
    ...
    for (int m = 0; m < M; m++)
        for (int n = 0; n < N; n++) {
            ...
            rt_checked++;
            if (got != ref) { if (nrt < 8) { first_rt[nrt][0] = m * N + n; first_rt[nrt][1] = got; first_rt[nrt][2] = ref; nrt++; } rt_bad++; }
            if (gemm(m, n, true) != ref) {                            // fault self-test
                if (nft < 8) { first_ft[nft][0] = m * N + n; first_ft[nft][1] = gemm(m, n, true); first_ft[nft][2] = ref; nft++; }
                fault_bad++;
            }
        }
    ...
    std::fprintf(stderr,
        ...
        rt_checked - rt_bad, rt_checked, rt_bad, rt_bad ? "" : " (clean)",
        fault_bad, rt_checked, discriminates ? "live" : "DEAD (no-op!)",
        layout_pass_ ? "LAYOUT PASS" : "LAYOUT FAIL");
    for (int i = 0; i < nrt; i++)
        std::fprintf(stderr, "    round-trip mismatch @ idx %d: got %d, expected %d\n",
                     first_rt[i][0], first_rt[i][1], first_rt[i][2]);
    if (std::getenv("MEMSIM_ACC"))
        for (int i = 0; i < nft; i++)
            std::fprintf(stderr, "    [fault] idx %d (m=%d,n=%d): permuted=%d vs correct=%d\n",
                         first_ft[i][0], first_ft[i][0] / N, first_ft[i][0] % N, first_ft[i][1], first_ft[i][2]);
    ...
    if (golden_z_) {
        ...
        for (int m = 0; m < M; m++)
            for (int n = 0; n < N; n++) {
                float acc = 0.f;
                for (int k = 0; k < K; k++)
                    acc += rdf(Abase + flatA(m, k)) * rdf(Bbase + flatB_os(k, n));
                model.push_back(acc);
        ...
        std::fprintf(stderr,
            "FP32 osCore vs golden (tile0, %dx%d): requant scale=%.4g, %d/%zu within %.0f%% -> %s\n",
        ...
        for (int i = 0; i < nfm; i++)
            std::fprintf(stderr, "    z mismatch @ (m=%d,n=%d): model=%.3f scale*golden=%.3f\n",
                         fm[i] / N, fm[i] % N, model[fm[i]], scale * gold[fm[i]]);
    }
    ...
    if (std::getenv("MEMSIM_DATAPATH") && golden_isc_ && golden_y_ &&
        ...
        layout_pass_ = layout_pass_ &&
            cmp_fp32_golden(model, gold, "isCore (out GEMM)", isN, 0.30, 0.0625, 0.05);
    }
    ...
    if (std::getenv("MEMSIM_DATAPATH") && golden_y_ && golden_z_ && golden_dtbc_ &&
        golden_sucx_ && dState_ && cfg_.seqLen % 16 == 0 && cfg_.dInner % 24 == 0) {
        ...
        for (int i = 0; i < Lq; i++)
            for (int d = 0; d < Din; d++) {
                ...
                for (int n = 0; n < dS; n++) {
                    double hv = h[(size_t)d * dS + n] * std::exp(rdf(Ab + d * dS + n) * dd) +
                                (BAt(i, n) * dd) * xid;
                    h[(size_t)d * dS + n] = hv;
                    hC += hv * CAt(i, n);
                }
    ...
    if (phase2_ && osc_dur_ && suc_dur_ && isc_dur_) {
        ...
        auto y_stale = [&](long sc) {
            double rstart = (double)sc / Ne * suc_dur_, prate = (double)suc_dur_ / Ne,
                   crate = (double)isc_dur_ / Ne;
            long stale = 0;
            for (long e = 0; e < Ne; e++) {
        ...
        auto run = [&](const char* lbl, long total, auto&& fn) {
            long lo = 1, hi = total, opt = total;
            while (lo <= hi) {
                long mid = lo + (hi - lo) / 2;
                if (fn(mid) == 0) { opt = mid; hi = mid - 1; } else lo = mid + 1;
            }
            ...
            std::fprintf(stderr,
                ...
                lbl, opt, total, below, s_below);
        };
        run("R10 (z, osCore->SUC)", M_i * osN, z_stale);
        run("R11 (y, SUC->isCore)", Ne, y_stale);
    }
    ...
    if (ports_[7].enabled) {
        ...
        std::fprintf(stderr,
            ...
            b.groups, b.clean_complete ? "PASS" : "FAIL",
            b.clean_poison_free ? "PASS" : "FAIL", b.clean_poison_count, b.groups,
            b.fault_detected ? "YES" : "NO", b.fault_poison_count,
