...
class SimWorld : public World {
   public:
    ...
    void set_goldens(uint32_t z, uint32_t y, uint32_t isc) {
        golden_z_ = z; golden_y_ = y; golden_isc_ = isc;
    }
    ...
    void set_ssm(uint32_t dState, uint32_t xProjDim, uint32_t bc_pad, uint32_t dtRankU) {
        dState_ = dState; xProjDim_ = xProjDim; bc_pad_ = bc_pad; dtRankUnroll_ = dtRankU;
    }
    ...
    void set_p1out(uint32_t dtbc, uint32_t sucx) { golden_dtbc_ = dtbc; golden_sucx_ = sucx; }
    ...
    struct TraceSeg { int engine; uint64_t start, end, ideal; };
