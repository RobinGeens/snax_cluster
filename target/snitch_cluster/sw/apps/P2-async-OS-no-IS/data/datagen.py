...
class DataGenerator(_oscore_datagen.DataGenerator):
    ...
    def _emit_p2_rings(self):
        """Async-ring extensions for THIS app only (runtime strided-DMA de-interleave, no scala
        change). The combined dt_BC L3 buffer is laid out per seqLenUnroll-window as
        [dt (su*dtRank B) | BC (su*2*dState B)], repeating every (su*xProjDim B). We:
          - R2 (dt): read a PACKED full dt buffer (windows abut at su*dtRank instead of su*xProjDim).
          - R7 (BC): ring nb_slots L-tiles. The SUC re-reads BC once per delaySU channel-group
            (broadcast = dInner_tile/delaySU), so the wrap walks nb_l_tiles*broadcast L-tile visits.
        Both within-window byte layouts are copied verbatim by the DMA, so only inter-window strides
        change. See docs/dataflow/09_async_tiling.md and 04_mamba_main.md.
        """
        FP8 = 8
        ...
        r2_tb = [self.dtRank * FP8 // self.switchcore_width, su, self.seqLen // su,
                 self.dInner_tile // self.convUnroll]
        ...
        r7_tb = [(2 * self.dState * FP8) // (2 * self.suc_serial_width_BC), su,
        ...
        conv_ring_tb = [conv_sub, self.nb_slots * win_per_l_tile,
                        (self.nb_l_tiles * cg_count) // self.nb_slots, 1]
