...
class DataGenerator(DataGeneratorBase):
    ...
    def run(self):
        ...
        self.lines_params.append(f"uint32_t xProjDim_tile = {self.xProjDim_tile};")
        self.lines_params.append(f"uint32_t dModel_tile = {self.dModel_tile};")
    ...
    def save_params(self):
        ...
        self.nb_n_tiles = self.kwargs["nb_n_tiles"]
        self.xProjDim_tile = self.xProjDim // self.nb_n_tiles
        self.dModel_tile = self.dModel // self.nb_n_tiles
    ...
    def check_tiling_constraints(self):
        ...
        # N_tile constraints: nb_n_tiles must divide both xProjDim and dModel,
        # and the resulting tile sizes must be compatible with the weight downsize factor.
        nn = self.nb_n_tiles
        assert self.xProjDim % nn == 0, (
            f"xProjDim ({self.xProjDim}) must be divisible by nb_n_tiles ({nn})"
        )
        assert self.dModel % nn == 0, (
            f"dModel ({self.dModel}) must be divisible by nb_n_tiles ({nn})"
        )
        wdf = self.weight_downsize_factor
        assert (self.xProjDim_tile * self.dInnerUnroll * FP8) % self.gemm_weight_width == 0, (
            f"xProjDim_tile ({self.xProjDim_tile}) * dInnerUnroll * FP8 must be "
            f"divisible by gemm_weight_width ({self.gemm_weight_width})"
        )
        assert (self.dModel_tile * self.dInnerUnroll * FP8) % self.gemm_weight_width == 0, (
            f"dModel_tile ({self.dModel_tile}) * dInnerUnroll * FP8 must be "
            f"divisible by gemm_weight_width ({self.gemm_weight_width})"
        )
    ...
    # Phase 1
    # =========================================================================
    def _build_p1_streamers(self, N_kern, dFinal=None):
        """Build P1 streamer bound/stride dict for an arbitrary K-step count N_kern
        and IS-core output dimension dFinal (defaults to full xProjDim).

        When dFinal < xProjDim (N-tiling), only the temporal BOUNDS of R12/R13/W3
        change. The temporal STRIDES of R12 keep the full xProjDim K-row stride
        because the weight tile in TCDM has full xProjDim per K-row.
        """
        if dFinal is None:
            dFinal = self.xProjDim
        downsized_dFinal = int(dFinal / self.weight_downsize_factor)
        ...
        return {
            ...
            "R13": (
                [
                    (self.seqLen // self.seqLenUnroll) * dFinal,
                    N_kern,
                ],
                [
    ...
    def build_Phase1_data(self):
        ...
        nn = self.nb_n_tiles
        N_tile = self.xProjDim_tile
        len_iscore_out_psum_tile = self.seqLen * N_tile * BF16 // 8
        len_iscore_out_final_tile = self.seqLen * N_tile * BF16 // 8
        ...
        tile_scalars = {
            ...
            "length_iscore_out_psum_tile": len_iscore_out_psum_tile,
            "length_iscore_out_final_tile": len_iscore_out_final_tile,
            "iscore_weight_n_offset": int(N_tile / self.weight_downsize_factor) * self.gemm_weight_width // 8,
            "iscore_bias_n_inner": N_tile * self.seqLenUnroll * BF16 // 8,
            "iscore_bias_n_src_stride": self.xProjDim * self.seqLenUnroll * BF16 // 8,
            "iscore_bias_n_count": self.seqLen // self.seqLenUnroll,
        }
        ...
        # N-tiled variants: same K-step configs but with N_tile instead of full xProjDim.
        # Only the temporal BOUNDS change; strides are shared with the default config
        # (R12 K-stride = full xProjDim because weight in TCDM has full xProjDim per K-row).
        self._emit_alt_bounds(mode_id, self._build_p1_streamers(K_i, dFinal=N_tile), "nTile")
        self._emit_alt_bounds(mode_id, self._build_p1_streamers(max(K_i - 1, 0), dFinal=N_tile), "nTile_finalLead")
        self._emit_alt_bounds(mode_id, self._build_p1_streamers(1, dFinal=N_tile), "nTile_finalStep")
    ...
    def build_Phase2_data(self):
        ...
        nn = self.nb_n_tiles
        dModel_tile = self.dModel_tile
        len_iscore_out_psum_tile_P2 = self.seqLen * dModel_tile * BF16 // 8
        ...
        tile_scalars = {
            ...
            "length_iscore_weight_tile": len_iscore_weight // nb,
            "length_iscore_out_psum_tile": len_iscore_out_psum_tile_P2,
            "iscore_weight_n_offset": int(dModel_tile / self.weight_downsize_factor) * self.gemm_weight_width // 8,
            "iscore_bias_n_inner": dModel_tile * self.seqLenUnroll * BF16 // 8,
            "iscore_bias_n_src_stride": self.dModel * self.seqLenUnroll * BF16 // 8,
            "iscore_bias_n_count": self.seqLen // self.seqLenUnroll,
        }
        ...
        # N-tiled P2 alt bounds: R12/R13/W3 with dModel_tile instead of full dModel.
        # Strides are shared (R12 K-stride = full dModel because weight in TCDM has
        # full dModel per K-row).  Only emit the three streamers whose bounds change.
        downsized_dModel_tile = int(dModel_tile / self.weight_downsize_factor)
        nTile_p2 = {
            "R12": (
                [downsized_dModel_tile, self.seqLen // self.seqLenUnroll, N_kern],
                [0],  # strides ignored by _emit_alt_bounds
            ),
            "R13": (
                [(self.seqLen // self.seqLenUnroll) * dModel_tile, N_kern],
                [0],
            ),
            "W3": (
                [(self.seqLen // self.seqLenUnroll) * dModel_tile, N_kern],
        ...
        self._emit_alt_bounds(mode_id, nTile_p2, "nTile")
