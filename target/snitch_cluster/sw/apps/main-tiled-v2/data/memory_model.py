...
#
# Standalone L1 TCDM memory model for the `main-tiled-v2` app.
#
# Two-phase Mamba with dInner-axis tiling AND N-axis tiling of iscore_out.
#
# Shared FULL (always live across all N_tile iterations):
#   oscore_in, iscore_out_P1_psum_tile, iscore_out_P1_final_tile
#
# Phase 1 ping-pong (×2 each):
#   oscore_weight_tile, conv_weight_tile, conv_bias_tile,
#   iscore_weight_tile (FULL xProjDim per K-row), conv_out_tile (DMA to L3)
#
# Phase 2: iscore_out_P2_psum_tile replaces P1 iscore buffers, overlays P1 PP.
#
...
def build_report(params: dict) -> MemoryReport:
    ...
    report = MemoryReport("main-tiled-v2", {
    ...
    shared_bufs = [
        ...
        ("iscore_out tile (P1/P2)",       iscore_shared),
    ]
    ...
    l3_bufs = [
        ...
        ("z (L3)",                   len_z),
        ("y (L3)",                   len_y),
        ("iscore_out_P1_final (L3)", len_iscore_out_P1_full),
        ("iscore_out_P2 (L3)",       len_iscore_out_P2_full),
    ]
