...
int main(void) {
    ...
    uint8_t* ptr_x_re    = p;
    p                    = _ALIGN64(p + M3_length_x_branch);  // OS A (flatA)
    uint8_t* ptr_x_im    = p;
    p                    = _ALIGN64(p + M3_length_x_branch);
    uint8_t* ptr_x_re_cv = p;
    p                    = _ALIGN64(p + M3_length_x_branch);  // IS A (ConvFormat)
    uint8_t* ptr_x_im_cv = p;
    p                    = _ALIGN64(p + M3_length_x_branch);
    uint8_t* ptr_w_re    = p;
    p                    = _ALIGN64(p + M3_length_w_branch);  // OS weights
    uint8_t* ptr_w_im    = p;
    p                    = _ALIGN64(p + M3_length_w_branch);
    uint8_t* ptr_w_re_is = p;
    p                    = _ALIGN64(p + M3_length_w_branch);  // IS weights
    uint8_t* ptr_w_im_is = p;
    p                    = _ALIGN64(p + M3_length_w_branch);
    uint8_t* ptr_rr      = p;
    p                    = _ALIGN64(p + M3_length_d);  // OS scratch (ConvFormat FP8)
    uint8_t* ptr_ii      = p;
    p                    = _ALIGN64(p + M3_length_d);
    // IS psum: seeded with b_im, then accumulates ri (call A) and ir (call B). BF16.
    uint8_t* ptr_P       = p;
    p                    = _ALIGN64(p + M3_length_cd);
    uint16_t* ptr_bf16_a = (uint16_t*)p;
    p                    = _ALIGN64(p + M3_length_bf16);  // real SIMD staging
    uint16_t* ptr_b_re   = (uint16_t*)p;
    p                    = _ALIGN64(p + M3_length_bias_mini_branch);  // real bias (conv-walk)
    uint8_t* ptr_out_re  = p;
    p                    = _ALIGN64(p + M3_length_out_branch);
    uint8_t* ptr_out_im  = p;
    p                    = _ALIGN64(p + M3_length_out_branch);
    ...
    for (int layer = 0; layer < 2; layer++) {
        ...
        for (uint32_t b = 0; b < M3_nBranches; b++) {
            ...
            if (snrt_global_core_idx() == 0) {
                ...
                set_streamer_csr((uint32_t)ptr_x_re, M3_R0_ss, M3_R0_tb, M3_R0_ts, M3_R0_en,         // R0
                ...
                set_simd_streamer_csr((uint32_t)ptr_rr, M3_R7_widen_ss, M3_R7_widen_tb, M3_R7_widen_ts,
                                      (uint32_t)ptr_ii, M3_R13_fp8_ss, M3_R13_fp8_tb, M3_R13_fp8_ts,
                ...
                set_simd_streamer_csr((uint32_t)ptr_bf16_a, M3_R7_bf16_ss, M3_R7_bf16_tb, M3_R7_bf16_ts,
                                      (uint32_t)ptr_b_re, M3_R13_bias_ss, M3_R13_bias_tb, M3_R13_bias_ts,
