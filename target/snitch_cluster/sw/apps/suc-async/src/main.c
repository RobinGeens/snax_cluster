...
//
// Isolated SUC with async tiling on BC, x, z and y. dt is kept full. See 12_suc_async.md
// (09_async_tiling.md for the ring working principle).
...
static inline void suc_ring_loop(uint8_t* bc_slot, const uint8_t* l3_dt_BC,  //
...
#undef _ALIGN64
    ...
    // Preload once: packed dt (all L windows) + first nb_slots BC L-tiles.
    if (snrt_is_dm_core()) {
    ...
    for (uint32_t tile = 0; tile < nb_tiles; tile++) {
        ...
        suc_ring_loop(ptr_BC_ring, M2_dt_BC, ptr_x_ring, x_l3_tile, ptr_z_ring, z_l3_tile, ptr_y_ring,
                      ptr_y_l3 + tile * M2_length_y_tile);
