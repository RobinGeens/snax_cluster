...
int burner() {
    // Hardcoded mode, see snax_simbacore_shell_wrapper.sv
    static const uint32_t CORE_BURNER_MODE = 0x40000000;
    ...
    uint32_t start_mcycle            = 0;  // Snitch mcycle for timing (perf counter often 0 in sim)
    uint32_t simbacore_cycles_phase1 = 0;
    if (snrt_global_core_idx() == 0) {
        printf("\nStarting program: SimbaCore Burner (Phase1 and Phase2)\n\n");
        ...
        set_simbacore_csr(CORE_BURNER_MODE | M1_PHASE1, seqLen, dModel, dInner, dtRank, xProjDim);
        // Start only SimbaCore, not the streamers
    ...
    if (snrt_global_core_idx() == 0) {
        ...
        set_simbacore_csr(CORE_BURNER_MODE | M2_PHASE2, seqLen, dModel, dInner, dtRank, dModel);
        // Start only SimbaCore, not the streamers
        _set_simbacore_start();
        wait_simbacore_and_streamer();
