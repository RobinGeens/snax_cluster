...
class DataGenerator(DataGeneratorBase):
    ...
    def save_params(self):
        ...
        # B1: oscore_in L-tiling (async refill of an nb_slots-slot ring).
        self.nb_l_tiles = self.kwargs["nb_l_tiles"]
        self.nb_slots = self.kwargs.get("nb_slots", 2)
        self.L_tile = self.seqLen // self.nb_l_tiles

        # BC bank-conflict fix via the AGU XOR swizzle (this app never carried bc_pad_banks).
        # See docs/dataflow/22_agu_xor_swizzle.md.
    ...
    def _swizzle_u16_image(self, data, base):
        ...
        for o in range(0, len(raw), 32):
            q = self._swz(base + o) - base
            out[q : q + 32] = raw[o : o + 32]
    ...
    def read_and_format_vector(self, mode_id, type, tensor_name):
        if self.bc_swizzle and mode_id == 1 and tensor_name == "iscore_bias":
            # P1 R13/W3 access iscore_out through the swizzle: pre-swizzle the bias init image.
    ...
    def check_tiling_constraints(self):
        ...
        # B1: oscore_in L-tiling constraints.
        nl = self.nb_l_tiles
        ns = self.nb_slots
        assert ns >= 2, f"nb_slots ({ns}) must be >= 2"
        assert nl >= ns and nl % ns == 0, f"nb_l_tiles ({nl}) must be >= nb_slots ({ns}) and a multiple of it"
        assert self.seqLen % nl == 0, f"seqLen ({self.seqLen}) must be divisible by nb_l_tiles ({nl})"
        assert self.L_tile % self.seqLenUnroll == 0, (
    ...
    def get_safe_to_start_delay(self, dInner: int):
        ...
        MARGIN = 1.0 if self.bc_swizzle else 0.2
    ...
    def build_Phase1_data(self):
        ...
        # main.c places iscore_out at align256(nb_slots * L_tile * dModel) from the 2 KiB-aligned
        # base: the swizzle-phase anchor for the bias image and the sampled check.
        oscore_ring_bytes = self.nb_slots * (self.L_tile * self.dModel * FP8 // 8)
        self._iscore_out_p1_addr = -(-oscore_ring_bytes // 256) * 256
        if self.bc_swizzle:
            assert len_iscore_out % 256 == 0
        ...
        tile_scalars = {
            ...
            # B1: oscore_in L-tiling (per-slot size, L3 stride between L-tiles, gauge ticks per L-tile).
            "length_oscore_in_l_tile": self.L_tile * self.dModel * FP8 // 8,
            "oscore_in_l_offset": self.L_tile * self.dModel * FP8 // 8,
            "oscore_in_l_tile_gauge_step": self.L_tile // self.seqLenUnroll,
        }
        ...
        # B1: R0 lTile variant — wraps an nb_slots-slot oscore_in ring via the stride-0
        # outer-loop trick. Bounds = [dModel, nb_slots*L_tile/seqLenUnroll, nb_l_tiles/nb_slots, K_i]:
        #   dim0 walks dModel; dim1 walks the nb_slots adjacent slots; dim2 (stride 0) rewinds
        #   to base for the next slot-group; dim3 (stride 0) rewinds across K-steps.
        ...
        self._emit_alt_bounds(mode_id, r0_lTile, "lTile")
        self.lines_params.append(f"uint32_t L_tile = {self.L_tile};")
        # nb_slots is emitted automatically as a params global (it is a params_in.hjson key).
    ...
    def build_Phase2_data(self):
        ...
        tile_scalars = {
            ...
            "length_iscore_weight_tile": len_iscore_weight // nb,
            # B1: oscore_in L-tiling (same as P1 — oscore_in is the shared x input).
            "length_oscore_in_l_tile": self.L_tile * self.dModel * FP8 // 8,
            "oscore_in_l_offset": self.L_tile * self.dModel * FP8 // 8,
            "oscore_in_l_tile_gauge_step": self.L_tile // self.seqLenUnroll,
        }
        ...
        # B1: R0 lTile variant for P2 (oscore_in / z-projection input). Same stride-0
