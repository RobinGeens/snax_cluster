...
int main(void) {
    ...
    for (int layer = 0; layer < 2; layer++) {
        ...
        for (uint32_t b = 0; b < M3_nBranches; b++) {
            ...
            for (uint32_t i = 0; i < nb_tiles + 1; i++) {
                ...
                if (snrt_is_dm_core()) {
                    if (i < nb_tiles) {
                        snrt_dma_start_1d(ptr_W_re_pp[buf],
                                          w_re_b_l3 + i * M3_length_w_branch_tile,
                                          M3_length_w_branch_tile);
                        snrt_dma_start_1d(ptr_W_im_pp[buf],
                                          w_im_b_l3 + i * M3_length_w_branch_tile,
                                          M3_length_w_branch_tile);
                    }
                    ...
                    if (i == 0 && b == 0) {
                        ...
                        snrt_dma_start_1d((uint8_t*)ptr_b_re_pp[bbuf],
                                          layer_b_re_l3, M3_length_bias_mini_branch);
                        snrt_dma_start_1d((uint8_t*)ptr_b_im_pp[bbuf],
                                          layer_b_im_l3, M3_length_bias_mini_branch);
                    }
                    snrt_dma_wait_all();
                }
            ...
            if (snrt_is_dm_core()) {
                if (b + 1 < M3_nBranches) {
                    int next_bbuf = (b + 1) & 1;
                    snrt_dma_start_1d(ptr_x_re_b,
                                      layer_x_re_l3 + (b + 1) * M3_length_x_branch,
                                      M3_length_x_branch);
                    snrt_dma_start_1d(ptr_x_im_b,
                                      layer_x_im_l3 + (b + 1) * M3_length_x_branch,
                                      M3_length_x_branch);
                    snrt_dma_start_1d((uint8_t*)ptr_b_re_pp[next_bbuf],
                                      layer_b_re_l3 + (b + 1) * M3_length_bias_mini_branch,
                                      M3_length_bias_mini_branch);
                    snrt_dma_start_1d((uint8_t*)ptr_b_im_pp[next_bbuf],
                                      layer_b_im_l3 + (b + 1) * M3_length_bias_mini_branch,
                                      M3_length_bias_mini_branch);
                }
                if (b > 0) {
                    int prev_obuf = (b - 1) & 1;
                    snrt_dma_start_1d(layer_out_re_l3 + (b - 1) * M3_length_out_branch,
                                      ptr_out_re_pp[prev_obuf], M3_length_out_branch);
                    snrt_dma_start_1d(layer_out_im_l3 + (b - 1) * M3_length_out_branch,
                                      ptr_out_im_pp[prev_obuf], M3_length_out_branch);
                }
            }
        ...
        if (snrt_is_dm_core()) {
            ...
            snrt_dma_start_1d(layer_out_re_l3 + (M3_nBranches - 1) * M3_length_out_branch,
                              ptr_out_re_pp[last_obuf], M3_length_out_branch);
            snrt_dma_start_1d(layer_out_im_l3 + (M3_nBranches - 1) * M3_length_out_branch,
                              ptr_out_im_pp[last_obuf], M3_length_out_branch);
            snrt_dma_wait_all();
        }
        snrt_cluster_hw_barrier();
    }
    ...
    if (snrt_global_core_idx() == 0) {
        ...
        printf("DMA latency hiding: wgt_tile=%s, simd=%s\n",
               _wgt_dma_done < _wgt_compute_done ? "hidden" : "STALL",
               _simd_dma_done < _simd_compute_done ? "hidden" : "STALL");
