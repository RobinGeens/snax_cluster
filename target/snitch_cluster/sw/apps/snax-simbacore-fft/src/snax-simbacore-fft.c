...
int test_isgemm() {
    ...
    if (snrt_is_dm_core()) {
        snrt_dma_start_1d(ptr_a, M4_dft_weight, M4_length_a);
        snrt_dma_start_1d(ptr_b, M4_dft_in, M4_length_b);
        // Initialize ptr_cd with zeros from zero memory
        snrt_dma_start_1d(ptr_cd, (void*)snrt_zero_memory_ptr(), M4_length_cd);
        snrt_dma_wait_all();
    }
    ...
    if (snrt_global_core_idx() == 0) {
        ...
        set_simbacore_csr(M4_ISGEMM, L1_padded0, 1, L1_padded1, 1, (dModel * L2));
        ...
        err += check_result_sample((uint8_t*)ptr_cd, M4_expected, M4_test_samples_expected,  //
                                   nb_test_samples, "out");

        printf("Test FFT: (%d x %d), channels=%d\n", L1_padded0, L1_padded1, dModel * L2);
