...
class DataGenerator(DataGeneratorBase):
    ...
    def build_data(self):
        ...
        # Input gather. dft_in is K-tile-major ([k1_tile][d][L2*L3][seqLenUnroll]) so R12_1's
        # K-stride lands on the next K-tile (see flattenDftInChipNEW). A slice's dM channels are
        # therefore NOT one contiguous chunk when L1 > seqLenUnroll: they sit at a per-K-tile
        # offset inside each of the L1/seqLenUnroll tiles. The per-slice DMA gathers them with a
        # 2-D transfer (repeat = in_ktile_count, src_stride = one full N-sweep). For L1 = seqLenUnroll
        # this is one tile (repeat=1) and collapses to the old contiguous 1-D copy.
