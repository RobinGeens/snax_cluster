...
class DataGenerator(DataGeneratorBase):
    ...
    def check_tiling_constraints(self):
        ...
        # iscore_out_P2 ring requires N_kern = dInner_tile/dInnerUnroll == 1 (one K-step per kernel),
        # so each P2 kernel sweeps the L-tiles exactly once and the ring can slide mid-kernel.
        assert self.dInner_tile == self.dInnerUnroll, (
            f"main-tiled-iscore needs N_kern=1: set nb_tiles = dInner/dInnerUnroll "
            f"({self.dInner}/{self.dInnerUnroll} = {self.dInner // self.dInnerUnroll}), got nb_tiles={nb}"
        )
        # L-tiling of the iscore_out_P2 psum (seqLen along M).
        assert self.seqLen % self.nb_l_tiles == 0, f"seqLen ({self.seqLen}) not divisible by nb_l_tiles"
    ...
    def _build_p1_streamers(self, N_kern):
        ...
        return {
            ...
            # R13/W3 = iscore_out_P1 async ring. Keep the (cycles-per-matrix, matrices) BC-pad inner
            # structure, but wrap the matrices axis into (matrices_per_slot, nb_slots, nb_l/nb_slots) with a
    ...
    def build_Phase1_data(self):
        ...
        tile_scalars = {
            ...
            # iscore_out_P1 ring: one slot = matrices_per_slot padded BC matrices; gauge ticks per
            # psum write-cycle, so a full L-tile = psum_cycles_per_matrix * matrices_per_slot cycles.
            "length_iscore_out_l_tile": self.p1_slot_bytes,
            "iscore_out_l_tile_gauge_step": self.psum_cycles_per_matrix * self.p1_matrices_per_slot,
        }
